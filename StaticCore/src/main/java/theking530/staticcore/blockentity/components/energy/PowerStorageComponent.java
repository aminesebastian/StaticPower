...
public class PowerStorageComponent extends AbstractBlockEntityComponent implements ISidedStaticPowerStorage {
	...
	public PowerStorageComponent(String name, double capacity, StaticPowerVoltage minInputVoltage, StaticPowerVoltage maxInputVoltage, double maxInputPower, CurrentType[] acceptableInputCurrents,
			StaticPowerVoltage voltageOutput, double maxPowerOutput, CurrentType outputCurrentType, boolean canAcceptExternalPower, boolean canOutputExternalPower) {
		super(name);
		...
		storage = new StaticPowerStorage(capacity, new StaticVoltageRange(minInputVoltage, maxInputVoltage), maxInputPower, acceptableInputCurrents, voltageOutput, maxPowerOutput, outputCurrentType,
	...
	public void postProcessUpdate() {
		...
		if (isPendingOverVoltageExplostion) {
			...
			if (SDMath.diceRoll(0.5)) {
				getLevel().addParticle(ParticleTypes.LARGE_SMOKE, getPos().getX() + randomX + 0.5, getPos().getY() + randomY + 0.5, getPos().getZ() + randomZ + 0.5, 0.0f, 0.0f, 0.0f);
			}
	...
	protected void checkUpgrades() {
		...
		UpgradeItemWrapper<Float> powerCapacityUpgrade = upgradeInventory.getMaxTierItemForUpgradeType(StaticCoreUpgradeTypes.POWER_CAPACITY.get());
		if (powerCapacityUpgrade.isEmpty()) {
			powerCapacityUpgradeMultiplier = 1.0f;
		} else {
			powerCapacityUpgradeMultiplier = (1.0f + (powerCapacityUpgrade.getUpgradeValue() * powerCapacityUpgrade.getUpgradeWeight()));
		}

		UpgradeItemWrapper<Float> powerTransferUpgrade = upgradeInventory.getMaxTierItemForUpgradeType(StaticCoreUpgradeTypes.POWER_TRANSFER.get());
		if (powerTransferUpgrade.isEmpty()) {
			powerOutputUpgradeMultiplier = 1.0f;
		} else {
			powerOutputUpgradeMultiplier = (1.0f + (powerTransferUpgrade.getUpgradeValue() * powerTransferUpgrade.getUpgradeWeight()));
		}
		...
		UpgradeItemWrapper<StaticPowerVoltage> powerTransformerUpgrade = upgradeInventory.getMaxTierItemForUpgradeType(StaticCoreUpgradeTypes.POWER_TRANSFORMER.get());
		if (powerTransformerUpgrade.isEmpty()) {
			...
			if (storage.getInputVoltageRange().maximumVoltage().isLessThan(powerTransformerUpgrade.getUpgradeValue())) {
				storage.setInputVoltageRange(new StaticVoltageRange(baseInputVoltageRange.minimumVoltage(), powerTransformerUpgrade.getUpgradeValue()));
			}
		}
	}
