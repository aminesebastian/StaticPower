...
public class BlockEntityPowerCable extends BlockEntityBase {
	...
	public BlockEntityPowerCable(BlockEntityTypeAllocator<BlockEntityPowerCable> allocator, BlockPos pos, BlockState state, boolean isIndustrial) {
		...
		double resistance = isIndustrial ? getTierObject().cablePowerConfiguration.cableIndustrialPowerResistance.get() : getTierObject().cablePowerConfiguration.cablePowerResistance.get();
	...
	public void process() {
		if (!isInsulated()) {
			if (!getLevel().isClientSide() && powerCableComponent.getEnergyTracker() != null) {
				...
				if (averageCurrent >= StaticCoreConfig.SERVER.electricalDamageThreshold.get()) {
					StaticPowerEnergyUtilities.applyElectricalDamageInArea(LivingEntity.class, level, damageRadius, averageCurrent, StaticCoreConfig.SERVER.electricalDamageMultiplier.get());
				}
			}
		}
