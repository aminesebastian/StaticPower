	...
	public void addProbeInfo(ProbeMode mode, IProbeInfo probeInfo, Player player, Level world, BlockState blockState, IProbeHitData data) {
		...
		if (processing.isPresent()) {
			if (processing.get().hasProcessingStarted()) {
				...
				MutableComponent suffix = GuiTextUtilities.formatNumberAsString(remaining).append(" ").append(Component.translatable("gui.staticcore.ticks_remaining"));
				...
				probeInfo.progress(0, 0,
						probeInfo.defaultProgressStyle().filledColor(JadePluginImplementation.MAIN_PROCESSING_COLOR.encodeInInteger())
								...
								.borderColor(SDColor.EIGHT_BIT_GREY.encodeInInteger()).suffix(Component.translatable("gui.staticcore.ticks_remaining")));
			}
		}
	}
