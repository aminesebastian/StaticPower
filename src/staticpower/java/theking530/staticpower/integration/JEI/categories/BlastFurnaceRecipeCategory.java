...
public class BlastFurnaceRecipeCategory extends BaseJEIRecipeCategory<BlastFurnaceRecipe> {
	...
	public BlastFurnaceRecipeCategory(IGuiHelper guiHelper) {
		super(guiHelper);
		locTitle = Component.translatable(ModBlocks.BlastFurnace.get().getDescriptionId());
		background = guiHelper.createBlankDrawable(120, 60);
		icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(ModBlocks.BlastFurnace.get()));
	...
	public void draw(BlastFurnaceRecipe recipe, IRecipeSlotsView recipeSlotsView, PoseStack matrixStack, double mouseX,
		...
		GuiDrawUtilities.drawSlot(matrixStack, 20, 20, 105, 17, 0);
		...
		if (experience > 0) {
			...
			GuiDrawUtilities.drawStringCentered(matrixStack, experienceString.getString(), 100, 10, 0.0f, 1f,
		...
		GuiDrawUtilities.drawStringCentered(matrixStack,
				GuiTextUtilities.formatTicksToTimeUnit(processingTicks).getString(), 110, 55, 0.0f, 1f,
				SDColor.EIGHT_BIT_GREY, false);
	}
	...
	public void setRecipe(IRecipeLayoutBuilder builder, BlastFurnaceRecipe recipe, IFocusGroup ingredients) {
		...
		builder.addSlot(RecipeIngredientRole.OUTPUT, 91, 19).addIngredient(PluginJEI.PROBABILITY_ITEM_STACK,
				recipe.getOutput());
		builder.addSlot(RecipeIngredientRole.OUTPUT, 107, 19).addIngredient(PluginJEI.PROBABILITY_ITEM_STACK,
