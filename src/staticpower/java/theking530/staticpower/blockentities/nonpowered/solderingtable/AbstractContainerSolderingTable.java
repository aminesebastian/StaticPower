...
public abstract class AbstractContainerSolderingTable<T extends AbstractSolderingTable> extends StaticPowerTileEntityContainer<T> implements IJEIReipceTransferHandler {
	...
	public AbstractContainerSolderingTable(ContainerTypeAllocator<? extends StaticCoreContainerMenu, ?> allocator, int windowId, Inventory inv, FriendlyByteBuf data) {
		this(allocator, windowId, inv, (T) resolveTileEntityFromDataPacket(inv, data));
	}

	public AbstractContainerSolderingTable(ContainerTypeAllocator<? extends StaticCoreContainerMenu, ?> allocator, int windowId, Inventory playerInventory, T owner) {
