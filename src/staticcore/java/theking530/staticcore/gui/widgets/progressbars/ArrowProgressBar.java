...
public class ArrowProgressBar extends AbstractProgressBar<ArrowProgressBar> {
	...
	public void renderWidgetBehindItems(PoseStack matrix, int mouseX, int mouseY, float partialTicks) {
		...
		if (flipped) {
			GuiDrawUtilities.drawTexture(matrix, StaticCoreGuiTextures.ARROW_PROGRESS_BAR, -getSize().getX(), getSize().getY(), getSize().getX(), 0, 0.0f, 0, 0.5f, 0.6875f, 1.0f);
			GuiDrawUtilities.drawTexture(matrix, StaticCoreGuiTextures.ARROW_PROGRESS_BAR, -getSize().getX() * visualCurrentProgresPercentage, getSize().getY(), getSize().getX(), 0.5f, 0.0f, 0.0f,
					0.0f, visualCurrentProgresPercentage * 0.6875f, 0.5f);
		} else {
			GuiDrawUtilities.drawTexture(matrix, StaticCoreGuiTextures.ARROW_PROGRESS_BAR, getSize().getX(), getSize().getY(), 0, 0, 0.0f, 0, 0.5f, 0.6875f, 1.0f);
			GuiDrawUtilities.drawTexture(matrix, StaticCoreGuiTextures.ARROW_PROGRESS_BAR, getSize().getX() * visualCurrentProgresPercentage, getSize().getY(), 0, 0 + 0.5f, 0.0f, 0.0f, 0.0f,
