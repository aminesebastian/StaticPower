...
public class ItemUtilities {
	...
	public static boolean filterItems(List<ItemStack> filterItems, ItemStack itemToCheck, boolean whitelist, boolean matchNBT, boolean matchOreDict, boolean matchMod) {
		...
		if (!match && matchOreDict) {
			for (ItemStack filterItem : filterItems) {
				for (TagKey<Item> filterItemTags : TagUtilities.getTags(filterItem)) {
					if (TagUtilities.matches(filterItemTags, itemToCheck.getItem())) {
						match = true;
						break;
					}
				}
	...
	public static boolean doStacksOverlapTags(ItemStack stack1, ItemStack stack2) {
		for (TagKey<Item> filterItemTags : TagUtilities.getTags(stack1)) {
			if (TagUtilities.matches(filterItemTags, stack2.getItem())) {
				return true;
			}
		}
		return false;
