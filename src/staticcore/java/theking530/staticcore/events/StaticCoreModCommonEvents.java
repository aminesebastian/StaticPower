...
public class StaticCoreModCommonEvents {
	...
	public static void commonSetupEvent(FMLCommonSetupEvent event) {
		event.enqueueWork(() -> {
			StaticCoreDataRegisterEvent staticCoreDataRegisterEvent = new StaticCoreDataRegisterEvent();
	...
	public static void registerCustomRegistries(@Nonnull NewRegistryEvent event) {
		...
		event.create(new RegistryBuilder<CableDestination>()
				.setName(StaticCoreRegistries.CABLE_DESTINATION_REGISTRY_KEY.location())
				.setIDRange(0, Integer.MAX_VALUE - 1));
		event.create(new RegistryBuilder<CableNetworkModuleType>()
				.setName(StaticCoreRegistries.CABLE_MODULE_REGISTRY_KEY.location())
				.setIDRange(0, Integer.MAX_VALUE - 1));
		event.create(new RegistryBuilder<ServerCableCapabilityType<?>>()
				.setName(StaticCoreRegistries.CABLE_CAPABILITY_REGISTRY_KEY.location())
				.setIDRange(0, Integer.MAX_VALUE - 1));
		event.create(
				new RegistryBuilder<ProductType<?>>().setName(StaticCoreRegistries.PRODUCT_TYPE_REGISTRY_KEY.location())
						.setIDRange(0, Integer.MAX_VALUE - 1));

		event.create(new RegistryBuilder<AttributeType<?>>()
				.setName(StaticCoreRegistries.ATTRIBUTE_REGISTRY_KEY.location()).setIDRange(0, Integer.MAX_VALUE - 1));
		event.create(new RegistryBuilder<AttributeModifierType<?>>()
				.setName(StaticCoreRegistries.ATTRIBUTE_MODIFIER_REGISTRY_KEY.location())
				.setIDRange(0, Integer.MAX_VALUE - 1));
		event.create(new RegistryBuilder<AttributeValueType<?>>()
				.setName(StaticCoreRegistries.ATTRIBUTE_VALUE_REGISTRY_KEY.location())
				.setIDRange(0, Integer.MAX_VALUE - 1));
		event.create(new RegistryBuilder<ItemAttributeType>()
				.setName(StaticCoreRegistries.ITEM_ATTRIBUTE_REGISTRY_KEY.location())
				.setIDRange(0, Integer.MAX_VALUE - 1));

		event.create(new RegistryBuilder<CompoundItemSlot>().setName(StaticCoreRegistries.ITEM_SLOT_KEY.location())
				.setIDRange(0, Integer.MAX_VALUE - 1));
	}
