		...
		if (!getLevel().isClientSide()) {
			...
			if (shouldBeStopping) {
				...
				if (soundStopCooldown <= 0) {
					...
					StaticCoreMessageHandler.sendToAllPlayersInDimension(StaticCoreMessageHandler.MAIN_PACKET_CHANNEL, getLevel(), syncPacket);
	...
	public void startPlayingSound(SoundEvent sound, SoundSource categoryIn, float volumeIn, float pitchIn, BlockPos pos, int blockRadius) {
		...
		if (getLevel().isClientSide()) {
			...
			if (soundStartCooldown == 0) {
				...
				StaticCoreMessageHandler.sendMessageToPlayerInArea(StaticCoreMessageHandler.MAIN_PACKET_CHANNEL, getLevel(), getPos(), blockRadius, syncPacket);
				shouldBeStopping = false;
