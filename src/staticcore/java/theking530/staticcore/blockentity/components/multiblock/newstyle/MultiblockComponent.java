...
public class MultiblockComponent<T extends BlockEntity> extends AbstractBlockEntityComponent {
	...
	public void onOwningBlockEntityBroken(BlockState state, BlockState newState, boolean isMoving) {
		...
		if (isMaster()) {
			pattern.onMultiblockBroken(this.state, getLevel());
		}
	}
	...
	public MultiblockComponent<T> setStateChangedCallback(Consumer<MultiblockState> stateChangedCallback) {
