...
public class MultiBlockCache<T extends BlockEntity> implements Iterable<MultiBlockEntry<T>> {
	...
	public MultiBlockFormationStatus update() {
		...
		if (controller == null) {
			return MultiBlockFormationStatus.FAILED;
		}
		...
		try {
			...
			Map<BlockPos, MultiBlockEntry<T>> discovered = WorldUtilities.bfsTraverseWorld(level,
					controller.getBlockPos(), (pos, state, entity) -> {
			...
			for (Entry<BlockPos, MultiBlockEntry<T>> entry : discovered.entrySet()) {
				if (!multiBlockEntries.containsKey(entry.getKey())) {
					...
					if (entity instanceof IMultiBlockComponent) {
						...
						comp.multiBlockValidated(entry.getValue());
					}
				}
			}
			...
			level.getProfiler()
					.push(String.format("MultiBlockCache.%1$s.CheckingWellFormed", controller.getClass().getName()));
			...
			if (status == MultiBlockFormationStatus.OK && initialBlockCount != multiBlockEntries.size()) {
				StaticCore.LOGGER.debug(String.format(
						"Multiblock with controller at location: %1$s updated originally with: %2$d blocks, now contains: %3$d blocks!",
						controller.getBlockPos(), initialBlockCount, multiBlockEntries.size()));
			} else if (status != currentStatus) {
				StaticCore.LOGGER.debug(String.format(
						"Multiblock with controller at location: %1$s failed to update with: %2$d blocks. Encountered status: %3$s.",
			...
			if (changed) {
				...
				for (BlockPos multiBlockEntry : multiBlockEntries.keySet()) {
					BlockState state = level.getBlockState(multiBlockEntry);
					level.markAndNotifyBlock(multiBlockEntry, level.getChunkAt(multiBlockEntry), state, state, 1 | 2,
							512);
				}
			}
			return status;
	...
	/**
	 * This value increments any time the multiblock structure changes.
	 * 
	 * @return
	 */
