...
public class StaticCoreKeyBindings {
	 ...
	 */
	public static final KeyBinding SLOT_LOCK = createKeyBinding("lock_slot", GLFW.GLFW_KEY_LEFT_CONTROL);
	/**
	 * Opens the research window.
	 */
	public static final KeyBinding OPEN_RESEARCH = createKeyBinding("open_research", GLFW.GLFW_KEY_R);
	/**
	 * Opens the production window.
	 */
	public static final KeyBinding OPEN_PRODUCTION = createKeyBinding("open_production", GLFW.GLFW_KEY_P);
	/**
	 * If held while clicking inside the side config tab, reset the config to
	 * default.
	 */
	public static final KeyBinding RESET_SIDE_CONFIGURATION = createKeyBinding("reset_side_configuration", GLFW.GLFW_KEY_R);
	...
	public static class KeyBinding {
		...
		public KeyBinding(String name, int key) {
			mapping = new KeyMapping("key.staticpower." + name, InputConstants.Type.KEYSYM, key,
					"key.categories.staticpower");
		}
