...
public class ResearchUnlock {
	...
	public ResearchIcon getIcon() {
		...
		if (type == ResearchUnlockType.CRAFTING) {
			CraftingRecipe recipe = StaticCoreRecipeManager.getRawRecipe(RecipeType.CRAFTING, target).orElse(null);
			if (recipe != null) {
	...
	public Recipe<?> getAsRecipe() {
		return StaticCoreRecipeManager.getRawRecipe(RecipeType.CRAFTING, getTarget()).orElse(null);
	}
