...
public class SelectedResearchWidget extends AbstractGuiWidget<SelectedResearchWidget> {
	...
	protected void renderWidgetBehindItems(PoseStack pose, int mouseX, int mouseY, float partialTicks) {
		// Draw background.
		GuiDrawUtilities.drawGenericBackground(pose, getSize().getX() + 4, getSize().getY() + 3.5f, -4, -4, 0.0f, new SDColor(0.25f, 0.5f, 1.0f, 1.0f));

		try {
			if (research != null) {
				for (int i = 0; i < description.size(); i++) {
					GuiDrawUtilities.drawStringLeftAligned(pose, description.get(i), 4, 33 + (i * 5.5f), 0f, 0.5f, SDColor.EIGHT_BIT_WHITE, true);
				}
				...
				GuiDrawUtilities.drawRectangle(pose, requirementsBgSize, 14, getSize().getX() - requirementsBgSize - 7, getSize().getY() - 19, -1000,
						new SDColor(0.0f, 0.0f, 0.0f, 0.35f));
				...
				for (int i = 0; i < requirementCount; i++) {
					...
					GuiDrawUtilities.drawString(pose, Integer.toString(remainingCount), getSize().getX() - 11 - xOffset, getSize().getY() - 13, 101, 0.5f, SDColor.EIGHT_BIT_YELLOW,
							true);
					xOffset += 14;
				}
				...
				if (unlocks.size() > 0) {
					GuiDrawUtilities.drawRectangle(pose, 0.4f, 14, getSize().getX() - 9f - xOffset, getSize().getY() - 19, 10, new SDColor(0, 0, 0, 0.3f));
