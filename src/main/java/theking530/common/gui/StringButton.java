...
public class StringButton extends AbstractGui implements MouseListener {
	...
	public void playSound() {
		if (TIMER == 1) {
			if (TILE_ENTITY != null) {
				// TILE_ENTITY.getWorldObj().playSound(TILE_ENTITY.xCoord, TILE_ENTITY.yCoord,
				// TILE_ENTITY.zCoord, "gui.button.press", 0.4F, 1.0F, CLICKED);
			}
		}
	}
	...
	public void drawButtonBase() {
		...
		if (CLICKED == true || HIGHLIGHT == true) {
			Minecraft.getInstance().getTextureManager().bindTexture(hover);
		} else {
	...
	public void sound(SoundHandler soundHandler) {
		// soundHandler.playSound(PositionedSoundRecord.func_147674_a(new
