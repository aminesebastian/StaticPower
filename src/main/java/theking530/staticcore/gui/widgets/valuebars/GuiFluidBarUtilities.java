...
public class GuiFluidBarUtilities {
	...
	private static void drawFluidBody(@Nullable PoseStack matrixStack, FluidStack fluid, int capacity, int amount, Vector2D origin, float zLevel, float width, float height) {
		...
		if (icon != null) {
			...
			RenderSystem.setShaderTexture(0, icon.getName());
...

			{
				...
				tes.vertex(origin.getX() + width + 0.1f, origin.getY() - depthHeightStart, zLevel)
						.color(fluidColor.getRed() * topColorTint, fluidColor.getGreen() * topColorTint, fluidColor.getBlue() * topColorTint, fluidColor.getAlpha()).uv(icon.getU1(), icon.getV0())
						.endVertex();
				...
				tes.vertex(origin.getX() - 0.1f, origin.getY() - depthHeightStart, zLevel)
						.color(fluidColor.getRed() * topColorTint, fluidColor.getGreen() * topColorTint, fluidColor.getBlue() * topColorTint, fluidColor.getAlpha()).uv(icon.getU0(), icon.getV0())
						.endVertex();
				tessellator.end();
			}
			...
			for (int i = 0; i < segmentsUsed; i++) {
				...
				tes.vertex(origin.getX(), origin.getY() - yMin, zLevel).color(fluidColor.getRed(), fluidColor.getGreen(), fluidColor.getBlue(), fluidColor.getAlpha()).uv(icon.getU0(), icon.getV0())
						.endVertex();
				tessellator.end();
			}
		}
