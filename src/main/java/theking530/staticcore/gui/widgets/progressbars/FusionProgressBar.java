...
public class FusionProgressBar extends AbstractProgressBar {
	...
	public void renderBehindItems(PoseStack matrix, int mouseX, int mouseY, float partialTicks) {
		...
		GuiDrawUtilities.drawTexturedModalRect(GuiTextures.FUSION_PROGRESS_BAR, null, screenSpacePosition.getX(), screenSpacePosition.getY() + 0.5f, 0.0f, getSize().getX(), getSize().getY(), 0.25f, 0.0f, 0.75f, 0.5f);

		if (visualCurrentProgresPercentage > 0) {
			GuiDrawUtilities.drawTexturedModalRect(GuiTextures.FUSION_PROGRESS_BAR, null, screenSpacePosition.getX(), screenSpacePosition.getY() + 0.5f, 0.0f, getSize().getX(), getSize().getY() * (visualCurrentProgresPercentage),
					0.25f, 0.5f, 0.75f, 0.5f + (0.5f * visualCurrentProgresPercentage));
		}
