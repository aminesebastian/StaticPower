...
public class BottleRecipeCategory extends BaseJEIRecipeCategory<BottleRecipe> {
	...
	public BottleRecipeCategory(IGuiHelper guiHelper) {
		...
		icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM, new ItemStack(ModBlocks.Bottler.get()));
	}
	...
	public void setRecipe(IRecipeLayout recipeLayout, BottleRecipe recipe, IIngredients ingredients) {
		...
		processingTimer = guiHelper.createTickTimer((int) StaticPowerConfig.SERVER.bottlerPowerUsage.get().longValue(), (int) StaticPowerConfig.SERVER.bottlerPowerUsage.get().longValue(), false);
	}
}
