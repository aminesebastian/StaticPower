...
public class BlockEntityBattery extends BlockEntityMachine {
	...
	private final StaticVoltageRange outputVoltageRange;
	private final double outputCurrentMaximum;
	...
	public BlockEntityBattery(BlockEntityTypeAllocator<BlockEntityBattery> allocator, BlockPos pos, BlockState state) {
		...
		outputVoltageRange = getTierObject().getBatteryOutputVoltageRange();
		outputCurrentMaximum = getTierObject().batteryMaximumOutputCurrent.get();

		// Add the power distributor.
		registerComponent(powerDistributor = new PowerDistributionComponent("PowerDistributor", powerStorage));
		powerStorage.setCapacity(getTierObject().batteryCapacity.get());
		powerStorage.setSideConfiguration(ioSideConfiguration);
		powerStorage.setInputVoltageRange(getTierObject().getBatteryInputVoltageRange());

		if (getTier() != StaticPowerTiers.CREATIVE) {
			powerStorage.setOutputVoltage(Math.floor((outputVoltageRange.minimumVoltage() + outputVoltageRange.maximumVoltage()) / 2));
			powerStorage.setMaximumOutputCurrent(Math.floor(outputCurrentMaximum / 2));
		} else {
	...
	public void process() {
		if (!getLevel().isClientSide()) {
			...
			if (getTier() == StaticPowerTiers.CREATIVE) {
				powerStorage.addPower(Double.MAX_VALUE, Double.MAX_VALUE, false);
			}
	...
	public void setMaximumOutputCurrent(double current) {
		if (current >= 0 && current <= outputCurrentMaximum) {
			powerStorage.setMaximumOutputCurrent(current);
		}
	}

	public void setOutputVoltage(double voltage) {
		if (outputVoltageRange.isVoltageInRange(voltage)) {
			powerStorage.setOutputVoltage(voltage);
		}
	}

	public void addMaximumOutputCurrent(double deltaCurrent) {
		double newCurrent = SDMath.clamp(powerStorage.getMaximumCurrentOutput() + deltaCurrent, 0, outputCurrentMaximum);
		powerStorage.setMaximumOutputCurrent(newCurrent);
	}

	public void addOutputVoltage(double deltaVoltage) {
		double newVoltage = outputVoltageRange.clampVoltageToRange(powerStorage.getVoltageOutput() + deltaVoltage);
