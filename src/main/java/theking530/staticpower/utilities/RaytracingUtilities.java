...
public class RaytracingUtilities {
	...
	public static Pair<Vector3d, Vector3d> getVectors(Entity entity) {
		...
		if (entity instanceof ServerPlayerEntity) {
			d3 = ((ServerPlayerEntity) entity).getAttribute(net.minecraftforge.common.ForgeMod.REACH_DISTANCE.get())
					.getValue();
		}
	...
	public static AdvancedRayTraceResult<BlockRayTraceResult> collisionRayTrace(BlockPos pos, Vector3d start,
			Vector3d end, Collection<AxisAlignedBB> boxes) {
		double minDistance = Double.POSITIVE_INFINITY;
		AdvancedRayTraceResult<BlockRayTraceResult> hit = null;
		int i = -1;

		for (AxisAlignedBB aabb : boxes) {
			AdvancedRayTraceResult<BlockRayTraceResult> result = aabb == null ? null
					: collisionRayTrace(pos, start, end, aabb, i, null);
			if (result != null) {
	...
	public static AdvancedRayTraceResult<BlockRayTraceResult> collisionRayTrace(BlockPos pos, Vector3d start,
			Vector3d end, AxisAlignedBB bounds, int subHit, Object hitInfo) {
