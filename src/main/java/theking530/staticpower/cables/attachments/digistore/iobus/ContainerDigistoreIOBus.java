...
public class ContainerDigistoreIOBus extends AbstractCableAttachmentContainer<DigistoreIOBusAttachment> {
	...
	static {
		if (FMLEnvironment.dist == Dist.CLIENT) {
			TYPE.setScreenFactory(GuiDigistoreIOBus::new);
		}
	}
	...
	public ContainerDigistoreIOBus(int windowId, Inventory inv, FriendlyByteBuf data) {
		this(windowId, inv, getAttachmentItemStack(inv, data), getAttachmentSide(data), getCableComponent(inv, data));
	}

	public ContainerDigistoreIOBus(int windowId, Inventory playerInventory, ItemStack attachment, Direction attachmentSide, AbstractCableProviderComponent cableComponent) {
