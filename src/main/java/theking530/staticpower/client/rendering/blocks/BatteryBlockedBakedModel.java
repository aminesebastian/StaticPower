...
public class BatteryBlockedBakedModel extends DefaultMachineBakedModel {
	...
	protected void renderQuadsForSide(@Nullable BlockState state, Builder<BakedQuad> newQuads, Direction side,
		...
		if (side.getAxis() == Direction.Axis.Y) {
			...
			BakedQuad newQuad = FaceBaker.bakeQuad(new Vector3f(5.0f, -0.01f, 5.0f), new Vector3f(11.0f, 16.01f, 11.0f),
					blockPartFace, sideSprite, side, ModelUtilities.IDENTITY, null, true,
			...
			if (side.getAxis() == Direction.Axis.X) {
				BakedQuad newQuad = FaceBaker.bakeQuad(new Vector3f(-0.01f, 5.0f, 5.0f),
				...
				BakedQuad newQuad = FaceBaker.bakeQuad(new Vector3f(5.0f, 5.0f, -0.01f),
