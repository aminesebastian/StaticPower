...
public class TankMachineBakedModel extends DefaultMachineBakedModel {
	...
	protected TextureAtlasSprite getSpriteForMachineSide(MachineSideMode mode, Direction side) {
		return Minecraft.getInstance().getTextureAtlas(InventoryMenu.BLOCK_ATLAS).apply(baseTexture);
	}
	...
	protected void renderQuadsForSide(@Nullable BlockState state, Builder<BakedQuad> newQuads, Direction side, BakedQuad originalQuad, MachineSideMode sideConfiguration) {
		...
		if (sideConfiguration != MachineSideMode.NA) {
			...
			if (side.getAxis() == Direction.Axis.Y) {
				...
				BakedQuad newQuad = FaceBaker.bakeQuad(new Vector3f(5.0f, -0.01f, 5.0f), new Vector3f(11.0f, 16.01f, 11.0f), blockPartFace, sideSprite, side,
				...
				if (side.getAxis() == Direction.Axis.X) {
					BakedQuad newQuad = FaceBaker.bakeQuad(new Vector3f(0, 5.0f, 5.0f), new Vector3f(16.0f, 11.0f, 11.0f), blockPartFace, sideSprite, side, ModelUtilities.IDENTITY,
							null, true, new ResourceLocation("dummy_name"));
					newQuads.add(newQuad);
				} else {
					BakedQuad newQuad = FaceBaker.bakeQuad(new Vector3f(5.0f, 5.0f, 0.0f), new Vector3f(11.0f, 11.0f, 16.0f), blockPartFace, sideSprite, side,
							ModelUtilities.IDENTITY, null, true, new ResourceLocation("dummy_name"));
					newQuads.add(newQuad);
				}
			}
