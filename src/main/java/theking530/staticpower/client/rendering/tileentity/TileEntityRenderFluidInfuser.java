...
public class TileEntityRenderFluidInfuser extends StaticPowerTileEntitySpecialRenderer<TileEntityFluidInfuser> {
	...
	public void renderTileEntityBase(TileEntityFluidInfuser tileEntity, BlockPos pos, float partialTicks,
		...
		if (!tileEntity.fluidTankComponent.isEmpty()) {
			...
			float lastSectionHeight = SDMath.clamp(filledPercentage - 0.8f, 0.0f, 0.2f) * 5 * 0.095f;

			drawFluidQuad(tileEntity.fluidTankComponent.getFluid(), matrixStack, buffer,
					new Vector3D(0.3125f, 0.186f, 0.001f), new Vector3D(0.376f, firstSectionHeight, 1.0f),
					new Vector4D(0.0f, 1.0f - filledPercentage, 1.0f, firstSectionHeight));

			drawFluidQuad(tileEntity.fluidTankComponent.getFluid(), matrixStack, buffer,
					new Vector3D(0.218f, 0.28f, 0.001f), new Vector3D(0.564f, secondSectionHeight, 1.0f),
					new Vector4D(0.0f, 1.0f - filledPercentage, 1.0f, secondSectionHeight));

			drawFluidQuad(tileEntity.fluidTankComponent.getFluid(), matrixStack, buffer,
					new Vector3D(0.3125f, 0.562f, 0.001f), new Vector3D(0.376f, lastSectionHeight, 1.0f),
					new Vector4D(0.0f, 1.0f - filledPercentage, 1.0f, lastSectionHeight));
			...
			if (!tileEntity.internalInventory.getStackInSlot(0).isEmpty()) {
				...
				IBakedModel itemModel = Minecraft.getInstance().getItemRenderer()
						.getItemModelWithOverrides(tileEntity.internalInventory.getStackInSlot(0), null, null);
				boolean render3D = itemModel.isGui3d();

				int forwardBlockLightLevel = WorldRenderer.getCombinedLight(tileEntity.getWorld(),
						tileEntity.getPos().offset(tileEntity.getFacingDirection()));

				if (render3D) {
					drawItemInWorld(tileEntity, tileEntity.internalInventory.getStackInSlot(0), TransformType.GUI,
