...
public class ModelFluidCapsule implements IModel {
	public static final ModelResourceLocation LOCATION = new ModelResourceLocation(
			new ResourceLocation(Reference.MOD_ID, "fluidcapsule"), "inventory");
	...
	public ModelFluidCapsule(@Nullable ResourceLocation baseLocation, @Nullable ResourceLocation liquidLocation,
	...
	public IBakedModel bake(IModelState state, VertexFormat format,
		...
		// if the fluid is a gas wi manipulate the initial state to be rotated 180? to
		// turn it upside down
		...
		if (baseLocation != null) {
			// build base (insidest)
			IBakedModel model = (new ItemLayerModel(ImmutableList.of(baseLocation))).bake(state, format,
					bakedTextureGetter);
		...
		if (coverLocation != null) {
			...
			builder.add(ItemTextureQuadConverter.genQuad(format, transform, 0, 0, 16, 16, NORTH_Z_COVER, cover,
					EnumFacing.NORTH, 0xffffffff));
			builder.add(ItemTextureQuadConverter.genQuad(format, transform, 0, 0, 16, 16, SOUTH_Z_COVER, cover,
					EnumFacing.SOUTH, 0xffffffff));
			if (particleSprite == null) {
				particleSprite = cover;
			}
		}

		return new BakedFluidCapsule(this, builder.build(), particleSprite, format, Maps.immutableEnumMap(transformMap),
				Maps.newHashMap());
	}

	/**
	 * Sets the liquid in the model. fluid - Name of the fluid in the FluidRegistry
	 * flipGas - If "true" the model will be flipped upside down if the liquid is a
	 * gas. If "false" it wont
	 * <p/>
	 ...
	 * The empty bucket/container fluid - A texture representing the liquid portion.
	 * Non-transparent = liquid cover - An overlay that's put over the liquid
	 * (optional)
	 * <p/>
	...
	public enum LoaderFluidCapsule implements ICustomModelLoader {
		...
		public boolean accepts(ResourceLocation modelLocation) {
			if (modelLocation.getResourceDomain().equals(Reference.MOD_ID)
					&& modelLocation.getResourcePath().contains("capsule")) {
				return true;
			}
			return false;
		}
	...
	private static final class FluidCapsuleCoverSprite extends TextureAtlasSprite {
		private final ResourceLocation bucket = new ResourceLocation("items/bucket_empty");
		private final ResourceLocation bucketCoverMask = new ResourceLocation(ForgeVersion.MOD_ID,
		...
		public boolean load(@Nonnull IResourceManager manager, @Nonnull ResourceLocation location,
			...
			IResource mask = LoaderFluidCapsule.getResource(
	...
	private static final class FluidCapsuleOverrideHandler extends ItemOverrideList {
		...
		public IBakedModel handleItemState(IBakedModel originalModel, ItemStack stack, @Nullable World world,
			...
			if (!model.cache.containsKey(name)) {
				...
				IBakedModel bakedModel = parent.bake(new SimpleModelState(model.getTransforms()), model.format,
						textureGetter);
	...
	private static final class BakedFluidCapsule extends BakedItemModel {
		...
		public BakedFluidCapsule(ModelFluidCapsule parent, ImmutableList<BakedQuad> quads, TextureAtlasSprite particle,
