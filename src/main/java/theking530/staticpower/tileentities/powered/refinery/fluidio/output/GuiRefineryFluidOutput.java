...
public class GuiRefineryFluidOutput extends StaticPowerTileEntityGui<ContainerRefineryFluidOutput, TileEntityRefineryFluidIO> {
	...
	public void initializeGui() {
		...
		registerWidget(new GuiFluidBarFromTank(getTileEntity().getTank(2), 104, 22, 16, 54, MachineSideMode.Output2, getTileEntity()));
		registerWidget(new GuiFluidBarFromTank(getTileEntity().getTank(3), 126, 22, 16, 54, MachineSideMode.Output3, getTileEntity()));
		registerWidget(new GuiFluidBarFromTank(getTileEntity().getTank(4), 148, 22, 16, 54, MachineSideMode.Output4, getTileEntity()));
	}
}
