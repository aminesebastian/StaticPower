	...
	public void process() {
		...
		if (processingComponent.getIsOnBlockState()) {
			...
			if (SDMath.diceRoll(0.25f)) {
				getLevel().addParticle(ParticleTypes.LARGE_SMOKE, getBlockPos().getX() + 0.5f + randomOffset, getBlockPos().getY() + 1.0f, getBlockPos().getZ() + 0.5f + randomOffset, 0.0f, 0.01f, 0.0f);
			}
		}
	}
