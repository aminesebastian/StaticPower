...
public class GuiTreeFarmer extends StaticPowerTileEntityGui<ContainerTreeFarmer, TileEntityTreeFarm> {
	...
	public void updateData() {
		...
		infoTab.addLine("desc1",
				new StringTextComponent("Farms trees in a " + TextFormatting.YELLOW + ((getTileEntity().getRadius() * 2) + 1) + "x" + ((getTileEntity().getRadius() * 2) + 1) + " radius."));
		...
		infoTab.addKeyValueTwoLiner("growth", new StringTextComponent("Current Growth Factor"),
				GuiTextUtilities.formatNumberAsString(getTileEntity().getGrowthBonus() * 100).appendString("%"), TextFormatting.GOLD);
	}
}
