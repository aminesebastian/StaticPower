...
public class GuiTestBattery extends StaticPowerTileEntityGui<ContainerTestBattery, BlockEntityTestBattery> {
	private TextButton inputUp;
	private TextButton inputDown;
	private TextButton outputUp;
	private TextButton outputDown;

	public GuiTestBattery(ContainerTestBattery container, Inventory invPlayer, Component name) {
		super(container, invPlayer, name, 176, 178);
	}

	@Override
	public void initializeGui() {
		registerWidget(new GuiPowerBarFromPowerStorage(getTileEntity().powerStorage, 75, 20, 26, 51));
...
//		getTabManager().registerTab(new GuiPowerInfoTab(getTileEntity().energyStorage).setTabSide(TabSide.LEFT), true);
	...
	public void buttonPressed(StandardButton button, MouseButton mouseButton) {
		...
		// Create the packet.
		NetworkMessage msg = new TestBatteryControlSyncPacket(getTileEntity().getInputLimit(), getTileEntity().getOutputLimit(), getTileEntity().getBlockPos());
