...
public class TestBatteryControlSyncPacket extends NetworkMessage {
	private long inputPerTick;
	private long outputPerTick;
	private BlockPos position;

	public TestBatteryControlSyncPacket() {
	}

	public TestBatteryControlSyncPacket(long inputPerTick, long outputPerTick, BlockPos pos) {
	...
	public void handle(Supplier<Context> ctx) {
		ctx.get().enqueueWork(() -> {
			if (ctx.get().getSender().getLevel().isAreaLoaded(position, 1)) {
				...
				if (rawTileEntity != null && rawTileEntity instanceof BlockEntityTestBattery) {
					BlockEntityTestBattery battery = (BlockEntityTestBattery) rawTileEntity;
