...
public class TileEntityVacuumChest extends TileEntityConfigurable implements MenuProvider {
	...
	public static final BlockEntityTypeAllocator<TileEntityVacuumChest> TYPE = new BlockEntityTypeAllocator<>((type, pos, state) -> new TileEntityVacuumChest(), ModBlocks.VacuumChest);
		...
		for (ItemEntity entity : droppedItems) {
			...
			if (InventoryUtilities.canFullyInsertItemIntoInventory(inventory, stack) && doesItemPassFilter(stack)) {
				...
				if (distance < 1.1 || (shouldTeleport && distance < getRadius() - 0.1f)) {
					...
					entity.remove();
					((ServerLevel) getLevel()).sendParticles(ParticleTypes.PORTAL, worldPosition.getX() + 0.5, (double) worldPosition.getY() + 1.0, (double) worldPosition.getZ() + 0.5, 1, 0.0D, 0.0D, 0.0D, 0.0D);
