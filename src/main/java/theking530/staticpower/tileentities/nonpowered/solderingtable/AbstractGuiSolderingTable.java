...
public class AbstractGuiSolderingTable<T extends AbstractSolderingTable, K extends AbstractContainerSolderingTable<T>>
		extends StaticPowerTileEntityGui<K, T> {
	...
	public void initializeGui() {
		...
		for (int y = 0; y < 3; y++) {
			for (int x = 0; x < 3; x++) {
				...
				FakeSlotButton slotButton = new FakeSlotButton(
						getTileEntity().patternInventory.getStackInSlot(x + (y * 3)), 62 + x * 18, 20 + y * 18,
						(button, mouse) -> {
	...
	public void updateData() {
		for (int y = 0; y < 3; y++) {
			for (int x = 0; x < 3; x++) {
				recipeFakeSlots.get(x + (y * 3))
						.setItemStack(getTileEntity().patternInventory.getStackInSlot(x + (y * 3)));
			}
		}
	}
	...
	protected void recipeButtonPressed(StandardButton button, MouseButton mouse, int x, int y) {
		...
		getMenu().getTileEntity().patternInventory.setStackInSlot(x + (y * 3),
		...
		StaticPowerMessageHandler.MAIN_PACKET_CHANNEL
				.sendToServer(new PacketSyncSolderingFakeSlotRecipe(getMenu().containerId, recipeItems));
	}
}
