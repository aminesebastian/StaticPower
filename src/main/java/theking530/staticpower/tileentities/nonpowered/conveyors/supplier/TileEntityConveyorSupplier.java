...
public class TileEntityConveyorSupplier extends TileEntityConfigurable {
	...
	public static final BlockEntityTypeAllocator<TileEntityConveyorSupplier> TYPE = new BlockEntityTypeAllocator<>(
			(type, pos, state) -> new TileEntityConveyorSupplier(pos, state), ModBlocks.ConveyorSupplier);
	...
	protected void postInit(Level world, BlockPos pos, BlockState state) {
		...
		if (facing == Direction.EAST) {
			importBox = new AABB(pos.getX() + conveyorLength, pos.getY() + 0.5, pos.getZ(), pos.getX() + 1.0,
					pos.getY() + 0.9, pos.getZ() + 1);
			conveyor.updateBounds(new AABB(pos.getX(), pos.getY() + 0.5, pos.getZ(), pos.getX() + conveyorLength,
					pos.getY() + 0.9, pos.getZ() + 1));
		} else if (facing == Direction.NORTH) {
			importBox = new AABB(pos.getX(), pos.getY() + 0.5, pos.getZ(), pos.getX() + 1, pos.getY() + 0.9,
					pos.getZ() + inverseConveyorLength);
			conveyor.updateBounds(new AABB(pos.getX(), pos.getY() + 0.5, pos.getZ() + inverseConveyorLength,
					pos.getX() + 1, pos.getY() + 0.9, pos.getZ() + 1));
		} else if (facing == Direction.SOUTH) {
			importBox = new AABB(pos.getX(), pos.getY() + 0.5, pos.getZ() + conveyorLength, pos.getX() + 1,
					pos.getY() + 0.9, pos.getZ() + 1);
			conveyor.updateBounds(new AABB(pos.getX(), pos.getY() + 0.5, pos.getZ(), pos.getX() + 1, pos.getY() + 0.9,
					pos.getZ() + conveyorLength));
		} else if (facing == Direction.WEST) {
			importBox = new AABB(pos.getX(), pos.getY() + 0.5, pos.getZ(), pos.getX() + inverseConveyorLength,
					pos.getY() + 0.9, pos.getZ() + 1);
			conveyor.updateBounds(new AABB(pos.getX() + inverseConveyorLength, pos.getY() + 0.5, pos.getZ(),
					pos.getX() + 1, pos.getY() + 0.9, pos.getZ() + 1));
		}

		// Make sure the front is output only.
		ioSideConfiguration.setWorldSpaceDirectionConfiguration(
				SideConfigurationUtilities.getDirectionFromSide(BlockSide.FRONT, facing), MachineSideMode.Output);
	}
